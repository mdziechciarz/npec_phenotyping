...
model_training_component = command(
    ...
    outputs={
        ...
        #     type="uri_folder", mode="rw_mount", description="Path to save the model"
        # ),
        "history": Output(
    ...
    code="src/",
    # command="python model_training.py --dataset-path ${{inputs.dataset_path}} --learning-rate ${{inputs.learning_rate}} --history-path ${{outputs.history}} --model-path ${{outputs.model_path}} --batch-size ${{inputs.batch_size}}",
...
# Example usage
# model_retraining_pipeline_instance = model_retraining_pipeline(
...
# # Submit the pipeline.
# pipeline_run = ml_client.jobs.create_or_update(model_retraining_pipeline_instance)
